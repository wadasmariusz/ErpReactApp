...
// import {mapInterWarehouseReceiptTypeToBadge} from "../../../../../components/maps/mapInterWarehouseReceiptTypeToBadge";
...
export const InterWarehouseReceiptsDataTable = () => {
  const { data } = useQueryContext();

  return (
    <DataTable header={headers}>
      {!!data?.length && data?.map(({ id, warehouse, status, createdAt, createdByUser }) => (
        <DataTableItem key={id} path={route["app.interWarehouseReceipt"](id)}>
          <div>PW-{id}</div>
